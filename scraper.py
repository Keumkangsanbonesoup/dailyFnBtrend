...
def summarize_with_ai(videos_data):
    """Gemini 2.5 Flash로 구체적인 F&B 상품명/메뉴명 키워드를 추출합니다."""
...
[분석 지시]
위 데이터를 분석하여, 현재 한국에서 **실제로 유행하거나 화제가 되고 있는 구체적인 F&B 아이템 5개**를 추출하세요.

중요:
- "디저트가 유행", "편의점 트렌드" 같은 **모호한 양상 표현은 절대 금지**.
- 반드시 **실제 상품명, 메뉴명, 또는 브랜드명**을 중심으로 작성할 것.
...
- `keywords` 배열에는 실제 검색에 쓸 수 있는 구체적인 단어만 3~5개 넣을 것.
  (예: ["탕후루", "과일사탕", "탕후루 만들기"], ["불닭", "편의점 신상", "마라맛"])
- `source_video`는 반드시 위 유튜브 데이터의 실제 url 중 하나를 선택할 것.
